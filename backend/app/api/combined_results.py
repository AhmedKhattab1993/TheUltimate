    ...
    try:
        ...
        for row in rows:
            result = CombinedScreenerBacktestRow(
                ...
                filter_price_vs_ma_enabled=row['filter_price_vs_ma_enabled'] if row['filter_price_vs_ma_enabled'] is not None else False,
                filter_price_vs_ma_period=row['filter_price_vs_ma_period'],
                filter_price_vs_ma_condition=row['filter_price_vs_ma_condition'],
                filter_rsi_enabled=row['filter_rsi_enabled'] if row['filter_rsi_enabled'] is not None else False,
                ...
                filter_gap_enabled=row['filter_gap_enabled'] if row['filter_gap_enabled'] is not None else False,
                ...
                filter_prev_day_dollar_volume_enabled=row['filter_prev_day_dollar_volume_enabled'] if row['filter_prev_day_dollar_volume_enabled'] is not None else False,
                filter_prev_day_dollar_volume=float(row['filter_prev_day_dollar_volume']) if row['filter_prev_day_dollar_volume'] else None,
                filter_relative_volume_enabled=row['filter_relative_volume_enabled'] if row['filter_relative_volume_enabled'] is not None else False,
...
async def get_combined_stats(
    ...
    """
    Get statistics for all combined results (not limited by pagination).
    """
    try:
        # Build the query with filters
        query = """
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN backtest_id IS NOT NULL THEN backtest_id END) as total_backtests,
            AVG(CASE WHEN total_return IS NOT NULL THEN total_return END) as avg_return,
            COUNT(CASE WHEN total_return > 0 THEN 1 END) as positive_returns,
            COUNT(CASE WHEN total_return IS NOT NULL THEN 1 END) as returns_count,
        ...
        # Calculate win rate
        win_rate = 0
        if row['returns_count'] and row['returns_count'] > 0:
            win_rate = (row['positive_returns'] / row['returns_count']) * 100
        
        return {
            "totalSymbols": row['total_symbols'] or 0,
            "totalBacktests": row['total_backtests'] or 0,
            "avgReturn": float(row['avg_return']) if row['avg_return'] else 0,
            "winRate": win_rate,
            "bestReturn": float(row['best_return']) if row['best_return'] else 0,
            "worstReturn": float(row['worst_return']) if row['worst_return'] else 0
        }
...
async def export_combined_results(
    ...
    """
    Export all combined results without pagination limit.
    """
    try:
        # Build the query with filters
        query = """
