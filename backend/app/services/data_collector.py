...
class DataCollector:
    ...
    def _validate_ohlc(self, bar: StockBar) -> bool:
        ...
        # Guard against absurdly large prices that exceed schema precision (numeric(10,2))
        price_fields = (bar.open, bar.high, bar.low, bar.close)
        for value in price_fields:
            if abs(value) >= 1e8:
                raise DataValidationError(f"Price {value} exceeds supported precision")
    ...
    async def _log_error(
        ...
        try:
            ...
            logger.debug(f"Failed to log error to database: {e}")
            ...
            if use_bulk_endpoint:
                ...
                async def streaming_callback(symbol: str, stock_data: StockData):
                    ...
                    if len(bars_buffer) >= self.batch_size:
                        try:
                            ...
                            logger.error(f"Error storing batch for {symbol}: {exc}")
                            await self._log_error(symbol, "storage_error", str(exc), target_date, target_date)
                            stats["errors"] += len(bars_buffer)
                        finally:
                            bars_buffer = []
                ...
                if bars_buffer:
                    try:
                        ...
                        logger.error(f"Error storing trailing batch: {exc}")
                        await self._log_error("bulk_batch", "storage_error", str(exc), target_date, target_date)
                        stats["errors"] += len(bars_buffer)
